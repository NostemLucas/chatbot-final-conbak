...
export default function SofiaApp() {
  ...
  const handleTextMessage = async (userMessage: string) => {
    ...
    try {
      ...
      setMessage(response.transcipt || "Lo siento, no pude procesar tu mensaje.");
    } catch (error) {
  ...
  const handleAudioMessage = async (audioBlob: Blob) => {
    ...
    try {
      ...
      setMessage(response.transcipt || "Lo siento, no pude procesar tu audio.");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-teal-900 to-slate-900 p-4">
      <div className="max-w-4xl mx-auto">
        <AgentAvatar isAnimating={isAnimating} />
