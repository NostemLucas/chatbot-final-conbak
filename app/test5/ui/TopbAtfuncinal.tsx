...
export default function VoiceTopBar({
  ...
  const isPlayingAudio = currentState === "speaking";
  ...
  // Efecto para manejar cambios de estado desde el padre
  useEffect(() => {
    switch (currentState) {
      case "listening":
      ...
      case "recording":
        ...
        break;
      case "processing":
        // El procesamiento ya está en marcha
        break;
      case "speaking":
        // El audio ya se está reproduciendo
        break;
      case "idle":
        stopAllActivities();
        break;
  ...
  const initializeContinuousListening = (): void => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (finalTranscript) {
        ...
        if (hasWakeWord && currentState === "listening") {
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if (currentState === "listening") {
        setTimeout(() => {
          onStateChange("listening"); // Reintentar
        }, 1000);
      }
    };
  ...
  const processAudio = async (audioBlob: Blob): Promise<void> => {
    try {
      ...
      onIntentDetected(data.dialogflow.intent, data.dialogflow.fulfillmentText);

      if (data.dialogflow.fulfillmentText) {
        onStateChange("speaking");
        await speakResponse(data.dialogflow.fulfillmentText);
        onStateChange("listening"); // Volver a escuchar
      } else {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3 min-w-[220px] justify-end">
            ...
            <div className="text-right">
              <div
                className={`text-sm font-medium transition-colors duration-300 ${getStateColor()}`}
              >
                {getStateText()}
              </div>
